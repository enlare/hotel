<?php include_once 'header.php'; ?>
<div class="container-small1 container-carousel1">
    <div id="carousel-main" class="carousel slide" data-ride="carousel">
        <!-- Wrapper for slides -->
        <div class="carousel-inner">
            <div class="item active">
                <img src="css/img/main/photo1.png" alt="...">
                <div class="carousel-caption">
                   <div class="slider-hotel-name clearfix">
                        Гостиница "Фишка2"
                    </div>
                    <div class="slider-hotel-row">
                        - 1-й Амбулаторный проезд, д 2/6 (м.Речной вокзал, Аэропорт)
                    </div>
                </div>
            </div>
            <div class="item ">
                <img src="css/img/main/photo2.png" alt="...">
                <div class="carousel-caption clearfix">
                    <div class="slider-hotel-name">
                        Гостиница "Фишка2"
                    </div>
                    <div class="slider-hotel-row">
                        - 1-й Амбулаторный проезд, д 2/6 (м.Речной вокзал, Аэропорт)
                    </div>
                </div>
            </div>
        </div>

        <!-- Controls -->
        <a class="left carousel-control" href="#carousel-main" data-slide="prev">
            <span class="icon-prev"></span>
        </a>
        <a class="right carousel-control" href="#carousel-main" data-slide="next">
            <span class="icon-next"></span>
        </a>
    </div>
</div>
<hr />
<div class="container-small">
    <div class="clearfix preview-wrapper">
        <div class="preview-row">
            <div class="col-md-4 col-main-preview">
                <a href="#" class="link-preview-wrapper">
                    <div>
                        <img src="css/img/main_preview.png" class="img-responsive "
                             alt=""/>
                    </div>
                    <div class="preview-description-header">
                        Андреев отель(м. Добрынинская)
                    </div>
                </a>
                <div class="preview-description">
                    Во всех номерах "Андреев отель" царит атмосфера уюта. В каждом номере нашей гостиницы 
                    есть всё необходимое для вашего отдыха.
                </div>
                <div class="preview-price">
                    Час от 500 руб. / Сутки от 3000 руб.
                </div>
                <a href="#" class="read-more">Подробнее</a>
            </div>
            <div class="col-md-4 col-main-preview">
                <a href="#" class="link-preview-wrapper">
                    <div>
                        <img src="css/img/main_preview.png" class="img-responsive "
                             alt=""/>
                    </div>
                    <div class="preview-description-header">
                        Андреев отель(м. Добрынинская)
                    </div>
                </a>
                <div class="preview-description">
                    Во всех номерах "Андреев отель" царит атмосфера уюта. В каждом номере нашей гостиницы 
                    есть всё необходимое для вашего отдыха.
                </div>
                <div class="preview-price">
                    Час от 500 руб. / Сутки от 3000 руб.
                </div>
                <a href="#" class="read-more">Подробнее</a>
            </div>
            <div class="col-md-4 col-main-preview">
                <a href="#" class="link-preview-wrapper">
                    <div>
                        <img src="css/img/main_preview.png" class="img-responsive "
                             alt=""/>
                    </div>
                    <div class="preview-description-header">
                        Андреев отель(м. Добрынинская)
                    </div>
                </a>
                <div class="preview-description">
                    Во всех номерах "Андреев отель" царит атмосфера уюта. В каждом номере нашей гостиницы 
                    есть всё необходимое для вашего отдыха.
                </div>
                <div class="preview-price">
                    Час от 500 руб. / Сутки от 3000 руб.
                </div>
                <a href="#" class="read-more">Подробнее</a>
            </div>
        </div>
        <div class="preview-row">
            <div class="col-md-4 col-main-preview">
                <a href="#" class="link-preview-wrapper">
                    <div>
                        <img src="css/img/main_preview.png" class="img-responsive "
                             alt=""/>
                    </div>
                    <div class="preview-description-header">
                        Андреев отель(м. Добрынинская)
                    </div>
                </a>
                <div class="preview-description">
                    Во всех номерах "Андреев отель" царит атмосфера уюта. В каждом номере нашей гостиницы 
                    есть всё необходимое для вашего отдыха.
                </div>
                <div class="preview-price">
                    Час от 500 руб. / Сутки от 3000 руб.
                </div>
                <a href="#" class="read-more">Подробнее</a>
            </div>
            <div class="col-md-4 col-main-preview">
                <a href="#" class="link-preview-wrapper">
                    <div>
                        <img src="css/img/main_preview.png" class="img-responsive "
                             alt=""/>
                    </div>
                    <div class="preview-description-header">
                        Андреев отель(м. Добрынинская)
                    </div>
                </a>
                <div class="preview-description">
                    Во всех номерах "Андреев отель" царит атмосфера уюта. В каждом номере нашей гостиницы 
                    есть всё необходимое для вашего отдыха.
                </div>
                <div class="preview-price">
                    Час от 500 руб. / Сутки от 3000 руб.
                </div>
                <a href="#" class="read-more">Подробнее</a>
            </div>
            <div class="col-md-4 col-main-preview">
                <a href="#" class="link-preview-wrapper">
                    <div>
                        <img src="css/img/main_preview.png" class="img-responsive "
                             alt=""/>
                    </div>
                    <div class="preview-description-header">
                        Андреев отель(м. Добрынинская)
                    </div>
                </a>
                <div class="preview-description">
                    Во всех номерах "Андреев отель" царит атмосфера уюта. В каждом номере нашей гостиницы 
                    есть всё необходимое для вашего отдыха.
                </div>
                <div class="preview-price">
                    Час от 500 руб. / Сутки от 3000 руб.
                </div>
                <a href="#" class="read-more">Подробнее</a>
            </div>
        </div>
    </div>
<!--    <div class="clearfix">
        <ul class="custom-pagination">
            <li><a href="#" class="pagination-item active-pagination-item"></a></li>
            <li><a href="#" class="pagination-item"></a></li>
            <li><a href="#" class="pagination-item"></a></li>
            <li><a href="#" class="pagination-item"></a></li>
        </ul>
    </div>-->
</div>
<hr />
<div class="container-small news-wrapper">
    <div class="clearfix">
        <div class="news-title">
            Наши новости
        </div>
        <div class="news-row clearfix">
            <div class="col-md-4 col-sm-4 col-xs-12  news-preview">
                <a href="#" class="link-preview-wrapper">
                    <div class="news-date">
                        <div class="news-day">30</div>
                        <div class="news-month">Apr 2014</div>
                    </div>
                    <div>
                        <img src="css/img/news.png" class="img-responsive" alt=""/>
                    </div>
                    <div class="preview-description-header news-description-header">
                        Открытие нового отеля в Марьино
                    </div>
                </a>
            </div>
            <div class="col-md-4 col-sm-4 col-xs-12 news-preview">
                <a href="#" class="link-preview-wrapper">
                    <div class="news-date">
                        <div class="news-day">30</div>
                        <div class="news-month">Apr 2014</div>
                    </div>
                    <div>
                        <img src="css/img/news.png" class="img-responsive" alt=""/>
                    </div>
                    <div class="preview-description-header news-description-header">
                        Открытие нового отеля в Марьино
                    </div>
                </a>
            </div>
            <div class="col-md-4 col-sm-4 col-xs-12 news-preview">
                <a href="#" class="link-preview-wrapper">
                    <div class="news-date">
                        <div class="news-day">30</div>
                        <div class="news-month">Apr 2014</div>
                    </div>
                    <div>
                        <img src="css/img/news.png" class="img-responsive" alt=""/>
                    </div>
                    <div class="preview-description-header news-description-header">
                        Открытие нового отеля в Марьино
                    </div>
                </a>
            </div>
        </div>
        <div>
            <a href="#" class="all-news-link">Все новости</a>
        </div>
    </div>
</div>

<?php include_once 'footer.php'; ?>